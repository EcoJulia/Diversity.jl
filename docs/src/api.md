...
| `AbstractPartition` | `_getsubcommunitynames` (any `AbstractVector{String}`, including one that builds each name when it is read) | `_countsubcommunities` (counts the subcommunity names), `_subsetpartition` (builds a `Subcommunities` from the names kept) |
